...
const pricing = {
  starter: { current: '₹499', regular: '₹999', suffix: '/ month', badge: '50% off' },
  business: { current: '₹739', regular: '₹1,499', suffix: '/ month', badge: '50% off' },
  shop: { current: '₹1,250', regular: '₹2,599', suffix: '/ month', badge: '50% off' },
  'one-time-starter': { current: '₹999', suffix: 'one-time', badge: 'Pay once' },
  'one-time-intermediate': { current: '₹1,499', suffix: 'one-time', badge: 'Pay once' },
  'one-time-high': { current: '₹2,739', suffix: 'one-time', badge: 'Pay once' },
};

document.querySelectorAll('.plan-card').forEach((card) => {
  const plan = pricing[card.dataset.planKey];
  if (!plan) return;

  const price = card.querySelector('.price');
  if (price) price.innerHTML = `${plan.regular ? `<span class="regular-price">${plan.regular}</span>` : ''}<span class="current-price">${plan.current}</span><small>${plan.suffix}</small><span class="discount-badge ${plan.badge === 'Pay once' ? 'one-time' : ''}">${plan.badge}</span>`;

  const setup = card.querySelector('.setup');
  if (setup) setup.innerHTML = plan.regular
    ? 'Setup fee: <small>discussed during consultation</small>'
    : 'One-time payment · <small>no monthly care — hosting is paid directly by you</small>';
});
...
if (bookingForm) bookingForm.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    alert('We couldn’t send your request right now. Please try again or contact Jugaad Labs on WhatsApp.');
  }
});
